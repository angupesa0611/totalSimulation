"""GAP — computational discrete algebra via the GAP system.

Simulation types:
  - group_properties: Order, elements, center, conjugacy classes
  - character_table: Character table computation
  - representation: Irreducible representations
  - space_group: Crystallographic space groups
  - stabilizer_codes: Quantum error correcting code generators
"""
...
# Well-known stabilizer code generators
_KNOWN_CODES = {
    "steane": {
...
class GAPTool(SimulationTool):
    name = "GAP"
    key = "gap"
    layer = "mathematics"

    SIMULATION_TYPES = {"group_properties", "character_table", "representation",
                        "space_group", "stabilizer_codes"}
    ...
    def validate_params(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        params.setdefault("group_type", "symmetric")
        params.setdefault("n", 5)
        params.setdefault("compute", "order")
        return params
    ...
    def run(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if sim_type == "group_properties":
            result = self._run_group_properties(params)
        elif sim_type == "character_table":
            result = self._run_character_table(params)
        elif sim_type == "representation":
            result = self._run_representation(params)
        elif sim_type == "space_group":
            result = self._run_space_group(params)
    ...
    def _run_group_properties(self, params):
    ...
    def _run_character_table(self, params):
        group_type = params.get("group_type", "symmetric")
        n = params.get("n", 4)
        group_code = _build_group(group_type, n)
...
"""
    ...
    def _run_representation(self, params):
        group_type = params.get("group_type", "symmetric")
        n = params.get("n", 3)
        group_code = _build_group(group_type, n)
...
"""
    ...
    def _run_space_group(self, params):
...
"""
    ...
    def get_default_params(self) -> dict[str, Any]:
        return {
            "simulation_type": "group_properties",
...
@celery_app.task(name="tools.gap_tool.run_gap", bind=True)
def run_gap(self, params: dict, project: str = "_default",
    ...
    save_result(self.request.id, "gap", result, project, label)
